...
class PandasChain:
    ...
    def display_chain(self):
        all_blocks = self.__chain + [self.__current_block]
    ...
    def __commit_block(self, block):
        ...
        hash = hashlib.sha256(str(str(self.__prev_hash) + str(self.__id) + str(dt.datetime.now()) + str(self.__seq_id) + str(block.get_simple_merkle_root)).encode('utf-8'))
    ...
    def display_block_headers(self):
        all_blocks = self.__chain.append(self.__current_block)
        print("\n")
        for block in all_blocks:
    ...
    def get_values(self):
        ...
        for block in self.__chain:
            values = values + block.get_values()
        return values

class Block:
    # 5 pts for constructor
    def __init__(self, seq_id=1, prev_hash=None):
        ...
        self.__block_hash = None #hash of the string concatenation of the previous block's
                                 #hash, the chains hash id, current date time, sequence id of the block and the root Merkle hash
        self.__status =  "UNCOMITTED"# Initial status. This will be a string.
    ...
    def display_header(self):
        print(str("sequence Id: "+self.__seq_id) + ", " + "status: " + self.__status + ", " +
              "hash: " + str(self.__block_hash) + ", " + "previous block hash: " + str(self.__prev_hash) + ", " +
              "merkle hash: " + str(self.__merkle_tx_hash) + ", " +
              "number of transations: " + str(len(self.__transactions.index))
              )
    ...
    def add_transaction(self, s, r, v):
        ...
        tx_hash =  hashlib.sha256(
            str(ts+s+r+str(v)).encode('utf-8')
        ).hexdigest()# Hash of timestamp, sender, receiver, value
        # {'Timestamp':ts,"Sender":s, "Receiver":r, "Value":v, "TxHash": str(tx_hash)}
        new_transaction =  pd.DataFrame(data = {'Timestamp':[ts],"Sender":[s], "Receiver":[r], "Value":[v], "TxHash": [str(tx_hash)]})# Create DataFrame with transaction data (a DataFrame with only 1 row)
    ...
    def set_block_hash(self, hash): # chain's hash id
    ...
    def get_simple_merkle_root(self):
        ...
        for index, row in self.__transactions.iterrows():
            tz_hashes + row["TxHash"]
        return hashlib.sha256(tz_hashes.encode('utf-8')).hexdigest()
