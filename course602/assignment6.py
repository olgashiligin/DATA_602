...
import plotly as py
py.tools.set_credentials_file(username='olgashi', api_key='52hWS9vyxKU8qMiJlpEp')
...
def exercise09():
    ...
    data = resp.json()
    fullDF = pd.DataFrame.from_dict(data)
    no_disclaimer = fullDF.drop("disclaimer", 1)
    df = no_disclaimer[:-2]
    data = [go.Scatter(
        x=df.index,
        y=df['bpi'])]

    plotly_url = plty.plot(data)
    print(df.head())
    print(df.tail())
    # ------ Place code above here /\ /\ /\ ------
...
def exercise12(n):
    ...
    checkerboard_matrix = np.array(([[1, 0] * n, [0, 1] * n] * n))
...
def exercise13(n):
    ...
    pddates = pd.date_range('1/1/2010', periods=n, freq='D')

    ints = np.random.randint(low=3, high=99, size=n)
    cum = np.cumsum(ints)
    s = pd.Series(data=np.asarray(cum), index=pddates)
    plt.plot(s)
    plt.show()
...
def exercise14(words):
    ...
    df = pd.DataFrame(data=words, index=words).apply(lambda row: len(row[0]), axis=1)
