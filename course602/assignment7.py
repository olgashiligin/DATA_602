...
def exercise02(new_observations):
    ...
    knn = KNN()
    knn.fit(iris_X, iris.target)
    KNN()


    iris_predictions = knn.predict(new_observations)
    predictions = []
    for pred in iris_predictions:
        predictions.append(iris_names[pred])
    return predictions
...
def exercise04():
    ...
    plotly_overfit_underfit_curve_url = plty.plot(data, filename='line-mode')
